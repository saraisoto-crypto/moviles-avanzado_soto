...
let estaciones: [Estacion] = [
    ...
    // ---------- LÍNEA 3 (Comas -> San Juan de Miraflores) ----------
    Estacion(nombre: "Comas", distrito: "Comas", lineas: [3]),
    Estacion(nombre: "Av. Túpac Amaru", distrito: "Independencia", lineas: [3]),
    Estacion(nombre: "Los Olivos", distrito: "Los Olivos", lineas: [3]),
    Estacion(nombre: "San Martín de Porres", distrito: "San Martín de Porres", lineas: [3]),
    Estacion(nombre: "Av. Tacna", distrito: "Cercado de Lima", lineas: [3]),
    Estacion(nombre: "Garcilaso de la Vega", distrito: "Cercado de Lima", lineas: [3]),
    Estacion(nombre: "Lince - Av. Arequipa", distrito: "Lince", lineas: [3]),
    Estacion(nombre: "San Isidro - Av. Arequipa", distrito: "San Isidro", lineas: [3]),
    ...
    // ---------- LÍNEA 5 (Surquillo - Miraflores - Chorrillos - Villa El Salvador) ----------
    Estacion(nombre: "Surquillo - Costa Sur", distrito: "Surquillo", lineas: [5]),
...
func normalizar(_ texto: String) -> String {
    return texto.folding(options: .diacriticInsensitive, locale: .current).lowercased()
}
...
func buscarPorNombre() {
    ...
    guard let texto = leerEntrada(), !texto.isEmpty else {
        print("Debes escribir un texto.")
        return
    }

    let objetivo = normalizar(texto)
    let resultados = estaciones.filter { normalizar($0.nombre).contains(objetivo) }
    ...
    print("\n--- Resultados ---")
    for e in resultados {
        let lineasTxt = e.lineas.map { nombreLinea($0) }.joined(separator: " y ")
...
func lineasPorLugar() {
    ...
    let objetivo = normalizar(texto)
    let resultados = estaciones.filter { normalizar($0.distrito).contains(objetivo) }
    ...
    let lineasEncontradas = Set(resultados.flatMap { $0.lineas }).sorted()
    let lineasTxt = lineasEncontradas.map { nombreLinea($0) }.joined(separator: " y ")
...
func queLineaTomar() {
    ...
    let resultados = estaciones.filter {
        normalizar($0.distrito).contains(objetivo) || normalizar($0.nombre).contains(objetivo)
    }
    ...
    for e in resultados {
        let lineasTxt = e.lineas.map { nombreLinea($0) }.joined(separator: " o ")
...
func mostrarTransferencias() {
    ...
    for e in transferencias {
        let lineasTxt = e.lineas.map { nombreLinea($0) }.joined(separator: " y ")
        print("• \(e.nombre) — \(e.distrito) — conecta \(lineasTxt)")
    }
}

func calcularRuta() {
    print("\nEscribe la estación de ORIGEN:")
    guard let origenTxt = leerEntrada(), !origenTxt.isEmpty else { return }

    print("Escribe la estación de DESTINO:")
    guard let destinoTxt = leerEntrada(), !destinoTxt.isEmpty else { return }
    ...
    guard let linea = lineaComun else {
        let transferencias = estaciones.filter { $0.lineas.count > 1 }
        let nombresTransferencia = transferencias.map { $0.nombre }.joined(separator: ", ")
        print("\n'\(origen.nombre)' y '\(destino.nombre)' no comparten una línea directa.")
        print("Podrías necesitar hacer transferencia en alguna de estas estaciones: \(nombresTransferencia)")
        return
    }
